...
from .shsa import SHSAttention, TopkAttention
...
class Block(nn.Module):
    def __init__(self, config, vis, alternate_partial_attn=False):
        ...
        if self.use_shsa:
            ...
            self.attn = TopkAttention(config, vis, dim=self.hidden_size, keep_rate=config.topk_attn)
        else:
...
class DepthwiseSeparableConv(nn.Module):
    def __init__(self, in_channels, out_channels, use_batchnorm=True, activation=True):
        super().__init__()
        ...
        self.dw_bn = nn.BatchNorm2d(in_channels) if use_batchnorm else nn.Identity()
        self.dw_act = nn.ReLU(inplace=True) if activation else nn.Identity()
        ...
        self.pw_bn = nn.BatchNorm2d(out_channels) if use_batchnorm else nn.Identity()
        self.pw_act = nn.ReLU(inplace=True) if activation else nn.Identity()
...
class EfficientUNetPPDecoderBlock(nn.Module):
    def __init__(
            self,
            in_channels,
            out_channels,
        ...
        super().__init__()
        self.up = nn.UpsamplingBilinear2d(scale_factor=2)
        fuse_in = in_channels + skip_channels
        self.fuse = nn.Sequential(
            nn.Conv2d(fuse_in, out_channels, kernel_size=1, bias=not use_batchnorm),
            nn.BatchNorm2d(out_channels) if use_batchnorm else nn.Identity(),
            nn.ReLU(inplace=True),
        )
        self.ds1 = DepthwiseSeparableConv(out_channels, out_channels, use_batchnorm=use_batchnorm)
        self.ds2 = DepthwiseSeparableConv(out_channels, out_channels, use_batchnorm=use_batchnorm)
...
class DecoderCup(nn.Module):
    def __init__(self, config):
        ...
        if self.config.decoder_blocks == 'effunetpp':
            blocks = [
                EfficientUNetPPDecoderBlock(in_ch, out_ch, sk_ch)
                for in_ch, out_ch, sk_ch in zip(in_channels, out_channels, skip_channels)
            ]
        elif self.config.decoder_blocks == 'effnet':
            blocks = [
                MBConvDecoderBlock(in_ch, out_ch)
                for in_ch, out_ch in zip(in_channels, out_channels)
            ]
        elif self.config.decoder_blocks == 'default':
            blocks = [
                DecoderBlock(in_ch, out_ch, sk_ch)
                for in_ch, out_ch, sk_ch in zip(in_channels, out_channels, skip_channels)
            ]
        else:
            raise ValueError(f"Unknown decoder block type: {self.config.decoder_blocks}. Supported types are: 'default', 'effunetpp', and 'effnet'.")
...
class VisionTransformer(nn.Module):
    ...
    def forward(self, x):
        if x.size()[1] == 1:
            x = x.repeat(1,3,1,1)
        encoded, attn_weights, features, kept_indices, orig_n = self.transformer(x)  # (B, n_patch, hidden)
        # If pruning occurred, scatter tokens back to the original grid length
        if kept_indices is not None:
            B, K, C = encoded.size()
            total_n = orig_n
            full = encoded.new_zeros(B, total_n, C)

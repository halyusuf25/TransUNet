...
class AWQViTSegQuantizer:
    ...
    def quantize(self) -> nn.Module:
        ...
        for block_idx, (blk, inputs) in enumerate(zip(blocks, block_inputs)):
            ...
            if self.args.use_se_block:
                if se_scales is not None and block_idx < len(se_scales):
                    scales_list = se_scales[block_idx]
                    if self.args.verbose:
                        print(f"[SE vs AWQ] Using SE scales for block {block_idx} with shape {scales_list.shape}.")
            
            else:
                scales_list = self._auto_scale_block_vit(blk, inputs)
                if self.args.verbose:
                    print(f"[SE vs AWQ] No SE scales for block {block_idx}; using AWQ scales with shape {scales_list[0][2].shape}.")
                    
            if self.args.verbose and self.args.use_se_block and se_scales is not None:
                scales_list = self._auto_scale_block_vit(blk, inputs)
    ...
    def _collect_block_inputs(self, blocks: List[nn.Module]) -> List[BlockCalibInputs]:
        ...
        for i_batch, batch in enumerate(self.calib_loader):
            ...
            if getattr(self.args, "use_se_block", False) and hasattr(self.model, "get_se_scales") and self._se_scales_cache is None:
                try:
    ...
    def _apply_scales(self, blk: nn.Module, scales_list) -> None:
        ...
        if getattr(self.args, "use_se_block", False):
            if torch.is_tensor(scales_list):
                se_vec = self._se_scale_to_vec(scales_list)
                scales_list = [
                    ("attention_norm", ("attn.query", "attn.key", "attn.value"), se_vec),
                    ("ffn_norm", ("ffn.fc1",), se_vec),
                ]
            else:
                raise ValueError(
                    "[SE vs AWQ] use_se_block is True, but SE scales were not provided. "
                    "Please ensure SE scales are available or set use_se_block to False."
                )
    ...
    def _search_best_scales(
        ...
        if self.args.use_se_block:
            Warning("Using SE scales for quantization")
            se_scale = self.model.get_se_scales(x)
            se_vec = self._se_scale_to_vec(se_scale)
            x_max = se_vec.to(dev, dtype=x_max.dtype).view(-1)

            # if torch.is_tensor(se_scale):
            #     se_vec = self._se_scale_to_vec(se_scale)
            # elif isinstance(se_scale, (list, tuple)):
            #     for item in se_scale:
            #         if torch.is_tensor(item):
            #             candidate = self._se_scale_to_vec(item)
            #             if candidate.numel() == awq_x_max.numel():
            #                 se_vec = candidate
            #                 break
            #     if se_vec is None and se_scale:
            #         first = se_scale[0]
            #         if torch.is_tensor(first):
            #             se_vec = self._se_scale_to_vec(first)

            # if se_vec is not None:
            # se_vec = se_vec.to(dev, dtype=awq_x_max.dtype).view(-1)
            # if se_vec.numel() != awq_x_max.numel():
            #     print(
            #         f"[SE vs AWQ] SE scale length {se_vec.numel()} "
            #         f"does not match activation dim {awq_x_max.numel()}; "
            #         "falling back to AWQ activation scale."
            #     )
            #     se_vec = None

        # x_max = se_vec if se_vec is not None else awq_x_max
        # else:
        #     x_max = _get_act_scale(x) # AWQ's per-channel mean abs activation
        else:
            Warning("Using AWQ activation scales for quantization")
            
        if self.args.verbose and self.args.use_se_block:
            print(f"Inside _search_best_scales dimension of x_max: {x_max.shape}")
            awq_x_max = _get_act_scale(x)
            se_scale = self.model.get_se_scales(x)
            se_vec = self._se_scale_to_vec(se_scale)
            se_vec = se_vec.to(dev, dtype=awq_x_max.dtype).view(-1)
            debug_se_vec = se_vec
            if debug_se_vec is None and hasattr(self.model, "get_se_scales"):
                try:
                    x_se_block = self.model.get_se_scales(x)
                except Exception as exc:
                    print(f"[SE vs AWQ] Failed to fetch SE scales for debug: {exc}")
                    x_se_block = None
                if x_se_block is not None:
                    debug_se_vec = self._se_scale_to_vec(x_se_block[0]).to(
                        awq_x_max.device, dtype=awq_x_max.dtype
                    )
            if debug_se_vec is not None:
                debug_se_vec = debug_se_vec.view(-1)
                if debug_se_vec.numel() == awq_x_max.numel():
                    print(f"SE scale vector shape: {debug_se_vec.shape}")
                    cos_sim = (awq_x_max * debug_se_vec).sum() / (
                        awq_x_max.norm() * debug_se_vec.norm()
                    ).clamp_min(1e-12)
                    print(
                        "Cosine similarity between AWQ x_max and SE scale vector: "
                        f"{cos_sim.item():.4f}"
                    )
                else:
                    print(
                        "[SE vs AWQ] SE scale vector shape mismatch: "
                        f"{tuple(debug_se_vec.shape)} vs {tuple(awq_x_max.shape)}"
                    )
